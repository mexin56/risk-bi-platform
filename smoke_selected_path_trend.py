...
time.sleep(3)
print(evaluate("document.body.innerText.includes('选中路径 · 近15天授信申请量趋势')"))
print(evaluate("document.body.innerText.includes('15天累计申请量')"))
print(evaluate("(() => { const title = [...document.querySelectorAll('div')].find((el) => el.textContent?.trim() === '选中路径 · 近15天授信申请量趋势'); if (!title) return 'TREND_TITLE_NOT_FOUND'; title.scrollIntoView({ block: 'start' }); return 'SCROLLED'; })()"))
time.sleep(1)
shot = call("Page.captureScreenshot", {"format": "png"})
out = Path(r"E:\agent\monitor\selected-path-trend-smoke.png")
