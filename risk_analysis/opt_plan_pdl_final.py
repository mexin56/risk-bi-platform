# -*- coding: utf-8 -*-
"""PDL 提额方案最终版 —— 额度使用率优先×风险档 提幅矩阵
提额条件: 评级 A/B/C 全部 + D级且fpd7_rate$<大盘(21.25%); E级不提
提幅: 使用率基础档(高300%/中180%/低80%) × 风险系数(10档 1.0~0.3), 300%封顶, ×k精确达标
"""
...
print(f"可提额 {df['是否可提额'].sum()} 行 | 提额客户 {df.loc[df['是否可提额'],'提额客户'].sum():,}")
...
def reason(r):
    ...
    return (f"评级={r['评级']}, fpd7_rate${r['fpd7_rate$']*100:.1f}%({r['风险档']}), {r['use_rate_group']}"
            f"(基础{USE_BASE[r['use_rate_group']]*100:.0f}%)×风险系数{r['风险系数']:.2f}×{k:.2f}→+{r['方案A幅度']*100:.0f}%, "
...
matrix_df[order] = matrix_df[order].astype(object)
...
summary = pd.DataFrame([
    ...
    {"模块": "约束规则", "指标": "提额客群范围", "数值": "A/B/C 全部 + D级低风险",
     "说明": f"D级且fpd7_rate$<大盘{cur_fpd7*100:.1f}%可提额({df.loc[(df['评级']=='D')&df['是否可提额'],'提额客户'].sum():,}客户); E级({df.loc[df['评级']=='E','提额客户'].sum():,})及D级高风险({df.loc[(df['评级']=='D')&~df['是否可提额'],'提额客户'].sum():,})不予提额"},
    {"模块": "提幅规则", "指标": "额度使用率优先", "数值": "高240% / 中170% / 低100%",
     "说明": "使用意愿优先(压降版): 高240% > 中170% > 低100%, 最大提幅压降至约240%"},
    {"模块": "提幅规则", "指标": "风险档系数", "数值": "10档 ×1.00 ~ ×0.30",
     "说明": "T1<2%×1.00 / T3 4-6%×0.90 / T5 8-10%×0.80 / T7 15-20%×0.60 / T9 25-30%×0.40 / T10>=30%×0.30"},
    {"模块": "提幅规则", "指标": "最终提幅", "数值": f"min(使用率基础×风险系数×{k:.2f}, 300%)",
...
for col in order:
    for r in range(2, ws_m.max_row + 1):
        cell = ws_m.cell(row=r, column=ws_m.cell(row=1, column=1).value and list(matrix_df.columns).index(col) + 1 or 1)
        pass
